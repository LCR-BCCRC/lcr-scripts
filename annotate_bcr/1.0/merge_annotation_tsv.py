...
normalisation steps are applied when --sample_id is given:
  ...
  * igseqr inputs: the AIRR 'sequence_id' column (which carries the
    transcript_id value) is renamed 'transcript_id' for consistency across
    igblast and V-QUEST outputs.
  * MiXCR inputs: a 'cloneId' column is extracted from the compound
    ...
    column matches the bare integer used in AIRR outputs.
"""
...
def _ordered_columns(all_cols):
    ...
    relative order.  Handles igblast-, V-QUEST-, igseqr-, and MiXCR-specific
    columns; all lookups go through present() so absent columns are silently
    skipped.
    """
    ...
    # --- Annotation quality flags (union of igblast and V-QUEST fields) ---
    order += present("locus", "stop_codon", "vj_in_frame", "v_frameshift",
                     "productive", "rev_comp", "complete_vdj", "complete_lvdjc",
                     "d_frame")

    # --- Gene calls (L first for V-QUEST, absent for igblast), then MiXCR best calls ---
    ...
    # --- Alignment quality: score, support, identity, cigar + positions ---
    # L gene (V-QUEST only; no support field)
...
def main():
    ...
    with open(args.annotation) as fh:
        ...
        if args.annot_key not in reader.fieldnames:
            sys.exit(
                ...
                f"       Check options.source_id_column in your module config."
            )
    ...
    with open(args.base) as fh_in, open(args.output, "w", newline="") as fh_out:
        ...
        if args.sample_id is not None:
            ...
            # the primary ID column matches the integer used in AIRR outputs.
            if (is_mixcr
                    and "sequence_id" in raw_header
